...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    if self.cell_type == 'rnn':
        dx, dh0, dWx, dWh, db = rnn_backward(dx,cell_cache)
    else:
        dx, dh0, dWx, dWh, db = lstm_backward(dx,cell_cache)
    ...
    dx,dw,db = affine_backward(dh0,affine_cache)
    grads['W_proj'] = dw
    grads['b_proj'] = db
    ...
    if self.cell_type == 'rnn':
        for n in xrange(N):
            ...
            for length in xrange(max_length):
                ...
                h,rnn_cache = rnn_step_forward(word_embedding_out,h,Wx,Wh,b)
